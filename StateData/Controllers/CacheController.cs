...
namespace StateData.Controllers
{
    public class CacheController : Controller
    {
        ...
        public async Task<ActionResult> PopulateCache()
        {
            ...
            HttpContext.Cache.Insert("pageLength", seData, seData, Cache.NoAbsoluteExpiration, Cache.NoSlidingExpiration, HandleNotification);

            return RedirectToAction("Index");
        }

        private void HandleNotification(string key, CacheItemUpdateReason reason, out object data, out CacheDependency dependency, out DateTime absoluteExpiry, out TimeSpan slidingExpiry)
        {
            Debug.WriteLine("Item {0} removed. ({1})", key, Enum.GetName(typeof(CacheItemUpdateReason), reason));
